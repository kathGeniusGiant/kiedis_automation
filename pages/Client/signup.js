...
export class clientSignupPage {
  ...
  async verifyElements() {
    await expect(
      this.page.getByRole('heading', { name: 'Sign up to Kiedis to continue recruiting these experts' })
    ).toBeVisible();

    await expect(
      this.page.getByRole('heading', { name: 'Enter your details' })
    ).toBeVisible();
  ...
  async checkAllFields(firstname, lastname, wrongEmail, correctEmail, newPassword, mismatchPwd) {
    ...
    /** COMPANY NAME — intentionally left blank */
  ...
  async signup(firstname, lastname, companyName, email, newPassword, confirmPassword) {
    ...
    await expect(
      this.page.locator('div').filter({
        hasText:
          'Email sent successfully! We have sent an email verification link to your email address. Please check your email and click the link to verify your account.'
      }).nth(2)
    ).toBeVisible();
